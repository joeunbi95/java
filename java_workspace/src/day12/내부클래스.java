...
public class 내부클래스 {
	...
	public static void main(String[] args) {
		...
		//외부클래스에서 내부클래스를 생성 => private으로 성언된 내부클래스는 접근 불가
...
class Outclass{
	...
	//Outclass 멤버변수로 추가하는 것과 같은 효과.(잘사용하지않음)
	class Inclass{//내부클래스
		int inNum=100;
...
//			System.out.println("Outclass Num="+num+"(외부클래스의 인스턴스 변수)");//error
		...
		static void sTest() {//static클래스의 static메서드
			System.out.println("Outclass sNum="+sNum+"(외부클래스의 static 변수 )");
			System.out.println("Inclass inNum="+sinNum+"(내부클래스의 static 변수)");
			//System.out.println("Outclass Num="+num+"(외부클래스의 인스턴스 변수)");
			//System.out.println("Inclass inNum="+inNum+"(내부클래스의 인스턴스 변수)");
